...
export default function MediaTracker() {
  ...
  const migrateDataToFirestore = useCallback(async (localData) => {
    ...
    try {
      ...
      // Batch upload met error handling per item
      ...
      hasMigrated.current = false; // Reset voor retry
    } finally {
      setLoading(false);
    }
  }, [user]);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // Fallback naar localStorage bij Firestore errors
      if (!isAnonymousUser(user)) {
        console.log("Firestore error, fallback naar localStorage");
        const localData = loadFromLocalStorage();
        setWatchlist(localData.watchlist || []);
        setWatching(localData.watching || []);
        setWatched(localData.watched || []);
  ...
  const sortResults = (results) => {
    ...
    switch (sortOption) {
      ...
      case "movie":
        sorted.sort((a, b) => {
          const typeA = a.media_type || '';
          const typeB = b.media_type || '';
  ...
  const updateStatus = async (item, newStatus) => {
    ...
    try {
      ...
      // Persisteer
      if (isAnonymousUser(user)) {
        const newWatchlist = newStatus === 'watching' || newStatus === 'watched' 
          ? watchlist.filter(i => i.id !== item.id) 
          : watchlist;
        const newWatching = newStatus === 'watching' 
          ? [updatedItem, ...watching.filter(i => i.id !== item.id)]
          : watching.filter(i => i.id !== item.id);
        ...
        // Check of het een temp ID is (van localStorage)
        if (item.id.toString().startsWith('temp_')) {
          // Dit is een nieuw item dat nog niet in Firestore staat
          ...
          // Update het item met het echte Firestore ID
          ...
          // Normaal update van bestaand document
  ...
  const deleteItem = async (id, currentListStatus) => {
    try {
      ...
      if (currentListStatus === 'watched') {
        setWatched(prev => {
          const updated = prev.filter(i => i.id !== id);
          if (isAnonymousUser(user)) {
            saveToLocalStorage(watchlist, watching, updated);
          }
          return updated;
        });
      }

      // Persisteer naar Firestore
      if (!isAnonymousUser(user)) {
        const itemRef = doc(db, "media_items", id);
